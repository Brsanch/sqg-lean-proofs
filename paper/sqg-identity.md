...
*Scope of claims (reader's guide).* **Theorem 1** (shear-vorticity identity, §2) is proved unconditionally and machine-verified in Lean 4 + mathlib in the companion repository. **Proposition 6.1** (selection rule, §6.1) is stated with a parity-based argument at formal Taylor-expansion level (not at theorem-grade rigor); the Lean library verifies the universal per-mode CZ precursor (`sqg_selection_rule_bound` in `SqgIdentity/Basic.lean`), while the pointwise $\kappa^2\delta^2$ refinement at the gradient maximum — which is what §9.6.3 needs — is the parity-based claim of Proposition 6.1. **Theorem 2** (SQG regularity, §9.6.3) is stated as conditional on two explicit hypotheses, labeled (H-strain) and (H-bdry), which concern the non-degeneracy of normal strain at the tracked curvature maximum and the boundedness of curvature on the boundary of a material segment, respectively. Paper §9.8 provides a reformulation (H-α) + (B-spec). Numerical evidence supporting these hypotheses is presented in §5 and §9, but none is derived unconditionally from the SQG dynamics at this time. The paper's contribution is the identity, the parity-based selection-rule refinement, and the firmed curvature-maximum-principle framework — together with an explicit accounting of what is rigorous, what is parity-heuristic, and what remains conditional.
...
**Machine-verified component.** Theorem 1 (shear-vorticity identity) is machine-verified in Lean 4 + mathlib in the accompanying repository (`sqg-lean-proofs`, `SqgIdentity/Basic.lean`). The **universal per-mode selection-rule bound** $\|\widehat{S_{nt} - \omega/2}(k)\| \leq |k|\,\|\widehat\theta(k)\|$ (and its integrated $\dot H^1$ form) is machine-verified as `sqg_selection_rule_bound` and `sqg_selection_rule_Hs1`; this is the universal Calderón–Zygmund precursor that Proposition 6.1 of this paper refines at the gradient maximum via a parity-based argument. **Proposition 6.1's pointwise $\kappa^2\delta^2$ refinement is not itself machine-verified**; its proof (Lemma 6.2) operates at formal-Taylor-plus-parity level of rigor. Theorem 2 is supplied as a *conditional* roadmap in the same formalization: given the hypotheses (H-strain) and (H-bdry) of §9.6.3 (reformulated as (H-α) + (B-spec) in §9.8), the material maximum principle (Proposition 9.9 in its firmed form with quadratic-in-curvature remainder), the BKM criterion, and the associated Sobolev calculus, the repository certifies uniform $\dot H^s$ Galerkin bounds and the full-range interpolation capstone `sqg_regularity_of_aubinLions_via_interpolation` for every $s \geq 0$, with zero `sorry` and no axioms added beyond mathlib. The hypothesis naming in the Lean source (`HasStrainLowerBound`, `HasBoundaryCurvatureBound`, `HasThermostatBound` in `RieszTorus.lean` §14) mirrors the paper §9.6.3/§9.8 labels.
