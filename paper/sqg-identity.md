...
*Scope of claims (reader's guide).* **Theorem 1** (shear-vorticity identity, §2) is proved unconditionally and machine-verified in Lean 4 + mathlib in the companion repository. **Proposition 6.1** (selection rule, §6.1) is stated with a parity-based argument at formal Taylor-expansion level (not at theorem-grade rigor); the Lean library verifies the universal per-mode CZ precursor (`sqg_selection_rule_bound` in `SqgIdentity/Basic.lean`), while the pointwise $\kappa^2\delta^2$ refinement at the gradient maximum — which is what §9.6.3 needs — is the parity-based claim of Proposition 6.1. **Theorem 2** (SQG regularity, §9.6.3) — **WITHDRAWN; see the Status banner** — was stated as conditional on two explicit hypotheses, labeled (H-strain) and (H-bdry), which concern the non-degeneracy of normal strain at the tracked curvature maximum and the boundedness of curvature on the boundary of a material segment, respectively; §9.8 provided a reformulation (H-α) + (B-spec). The 2026-05-29 audit found the conditional chain circular and the §9 material-maximum-principle mechanism contradicted by this paper's own numerics, so §§3–6 and 9 are retained for the record and should be read as heuristic mechanism, not proof. The paper's standing contribution is the identity (§2) and the closed-form curvature correction (§8), together with an explicit accounting of what is rigorous, what is parity-heuristic, and what is withdrawn.
...
Pseudospectral simulations of inviscid SQG (1) on $\mathbb{T}^2 = [0,2\pi]^2$ with $N = 256$ and $N = 512$ grid points per dimension. Fourth-order Runge-Kutta time integration, $2/3$-dealiasing, $\Delta t = 5 \times 10^{-4}$ ($N = 512$; see Appendix for the general choice). Initial condition: multimode $\theta_0 = \sin x\sin y + 0.5\cos 3x\sin 2y + 0.3\sin 2x\cos y$.
...
**Proposition 6.1 (Selection rule, parity-based refinement).** *Let $\theta \in C^\infty(\mathbb{T}^2)$ and let $x^*$ be a gradient maximum of $\theta$, with level-set curvature $\kappa$ at $x^*$ and front width $\delta = \|\theta\|_\infty / \|\nabla\theta\|_\infty$. Assume the gradient-maximum conditions $\theta_{nn}(x^*) = \theta_{nt}(x^*) = 0$ of Proposition 9.1. Then the near-field contribution to the normal strain satisfies*

$$|nSn_{\mathrm{near}}(x^*)| \;\leq\; C\,\kappa^2\,\delta^2\,\|\nabla\theta\|_\infty \;=\; C\,\kappa^2\,\|\theta\|_\infty^2\,/\,\|\nabla\theta\|_\infty, \tag{S}$$

*where $C$ is a constant depending only on the CZ kernel, $\|\Theta'''\|_\infty$ of the 1-D profile, and the cutoff radius $R$; and $nSn_{\mathrm{near}}$ denotes the contribution to $nSn(x^*)$ from the near-field shell $|y - x^*| < R\delta$ of the Riesz velocity's kernel (Lemma 6.2).*

*Argument.* Near $x^*$ the front is one-dimensional up to curvature $O(\kappa)$. The Riesz velocity's CZ kernel $K_f$ is even–even in $(n,t)$; three of the four parity sectors of the non-1D perturbation $\Delta\theta = \theta - \theta_{1\mathrm{D}}$ integrate to zero exactly. The surviving $(ee)$ sector's first-order-in-curvature contribution has angular average zero on each radial shell (by the vanishing angular mean of the CZ kernel's $(ee)$ part), so it does not contribute at that order. The leading surviving contribution is at second order in curvature, using $\theta_{nn}(x^*) = \theta_{nt}(x^*) = 0$ (Prop 9.1) to remove the $n^1$ coefficient of $\Theta'(n)$, giving the $\kappa^2\delta^2$ bound. Full derivation with constants: Lemmas 6.2, 6.3. Cf. Córdoba 1998 [4] for the unconditional $O(\kappa A)$ form.

**Remark (rigor level and relation to Lean).** Proposition 6.1 is stated as an *unconditional bound* on a specific near-field functional (not a regularity result). Its proof (Lemma 6.2 below) operates at the level of formal Taylor expansion plus parity-sector cancellation, with constants tracked dimensionally rather than sharply; it is tighter than the rigorous Córdoba [4] $O(\kappa A)$ bound and is the ingredient that makes bounded curvature suffice in §9.6.3. **Proposition 6.1 itself is not machine-verified**: the companion Lean library verifies the *universal per-mode CZ bound* $\|\widehat{S_{nt} - \omega/2}(k)\| \leq |k|\,\|\widehat\theta(k)\|$ (see `sqg_selection_rule_bound` in `SqgIdentity/Basic.lean`), which Proposition 6.1 refines at the gradient maximum via the parity argument. The `MaterialMaxPrinciple` chain in `SqgIdentity/RieszTorus.lean` §14 takes the *integrated* $\dot H^1$ form of the per-mode bound as one of its spectral inputs, not the pointwise $\kappa^2\delta^2$ refinement.

### 6.2 Self-consistent bound

The curvature budget (10) with $F_\text{ext} \leq C_2 G$ (verified in §5.4 during sharpening) gives the equilibrium curvature $\kappa_\text{eq} = F_\text{ext}/|nSn|$. Combined with the selection rule (S) of Proposition 6.1:
...
The same parity argument applies verbatim to the normal-strain multiplier $m_{nn}(k) = -(|k|/2)\sin(2\varphi_k)$, which is $(oo)$ in $(k_n, k_t)$, hence dual to $(oo)$ in $(n, t)$ on the physical side; the surviving sector is again the $O(\kappa^2)$ one, giving $|nSn_\text{near}(x^*)| \leq C\kappa^2\,A^2/G$ as stated in Proposition 6.1. $\square$

**Remark 6.2.1 (where the gradient-maximum conditions are essential).** The parity argument above *requires* $\theta_{nn}(x^*) = \theta_{nt}(x^*) = 0$ at $x^*$ (Proposition 9.1): without them, the 1-D profile $\Theta$ would contain an $n^1$ term in $\Theta'(n)$, and the $(ee)$ sector would receive an $O(\kappa)$ (rather than $O(\kappa^2)$) contribution. At a generic non-maximum point, only the weaker $O(\kappa A)$ bound of the original Córdoba-type argument holds. This is why Proposition 6.1 is stated at the gradient maximum $x^*$ specifically, and why the bootstrap of §6.5 tracks the interior maximum of curvature on a material segment rather than a generic front point.
...
The selection rule ($|nSn| \leq C\kappa^2\delta^2 G$) was known implicitly from the work of Constantin, Majda, and Tabak [1] and Córdoba [4], though not stated in this form. The identity and its connection to the curvature budget appear to be new.
...
**Remark (relation to (H-α)).** Conjecture 9.4 and the thermostat hypothesis (H-α) of §9.8 are logically distinct sufficient conditions for regularity, and the present paper does not derive one from the other. (H-α) combined with the spectral-to-pointwise bridge (B-spec) implies $|nSn(x^*)|$ bounded (Prop 9.11), which in turn implies the local $L^2$ bound (42) of Conjecture 9.4; in that indirect sense (H-α) + (B-spec) is stronger. Direct logical implication of Conjecture 9.4 from (H-α) alone — without (B-spec) — is not established here. Conjecture 9.4 is retained as an independent empirical observation from the §9.4 data and motivates the "battery" heuristic; the proof chain of §9.6.3 and §9.8 does not invoke it.
...
**Gap status (explicit).** The sign of $-8|nSn| + CG$ determines whether the gate damping wins in (52). If $|nSn| \geq CG/8$ (a fixed fraction of the CZ bound), the damping dominates. However, the regime of interest for the proof is $|nSn| \ll G$ (sub-CZ scaling). With $|nSn| \approx 0.86$ and $G$ growing, the term $CG$ eventually dominates for any fixed $C > 0$. **We record this as an unresolved gap: the Sobolev route of §9.5.1–§9.5.2 does not close on its own, and §9.6 (the material maximum principle) is the route actually used in Theorem 2.**
...
**Consequence for the main text.** Theorem 2 as stated in §9.6.3 is conditional on (H-strain) + (H-bdry) via the material maximum principle. The Sobolev-damping route of §9.5.2 is presented as an independent heuristic pathway that would, if closed, yield the same conclusion by a different chain; its failure does not affect the conditional validity of Theorem 2.
...
*The rest of §9.6 proceeds **under** (H-bdry); Theorem 2 below is correspondingly conditional.*
...
**Theorem 2 (SQG regularity, conditional).** *Assume (H-strain) and (H-bdry). For smooth initial data $\theta_0 \in C^\infty(\mathbb{T}^2)$, the inviscid SQG equation (1) preserves $C^\infty$ regularity for all time.*
...
The conditional proof chain for Theorem 2:
...
9. He, S., Kiselev, A. (2021). Small-scale creation for solutions of the SQG equation. *Duke Math. J.* 170(5), 1027–1041. arXiv:1903.07485.
10. Kiselev, A., Ryzhik, L., Yao, Y., Zlatoš, A. (2016). Finite time singularity for the modified SQG patch equation. *Ann. Math.* 184(3), 909–948. arXiv:1508.07613.
11. Jeong, I.-J., Kim, J. (2024). Strong ill-posedness for SQG in critical Sobolev spaces. *Analysis & PDE* 17, 133–170. arXiv:2107.07739.
12. Misiołek, G., Vu, X.-T., Yoneda, T. (2026). Regularity of solution maps of the generalized surface quasi-geostrophic equations. arXiv:2603.12944.

---

## Appendix: Numerical Methods

**Solver.** Pseudospectral on $[0, 2\pi]^2$ with $N \times N$ collocation points. Dealiasing: 2/3-rule. Time stepping: classical RK4 with $\Delta t = \min(5\times 10^{-4},\ 1.5/N)$; at $N = 512$ the $5\times 10^{-4}$ branch is binding. Inviscid ($\kappa = 0$). Implementation: `noethersolve/sqg_solver.py` (SQGSolver with `evolve_rk4`).
...
**Machine-verified component (revised 2026-07-02 to complete the 2026-05-29 withdrawal; see Status banner).** Theorem 1 (shear-vorticity identity) is machine-verified in Lean 4 + mathlib in the accompanying repository (`sqg-lean-proofs`, `SqgIdentity/Basic.lean`). The **universal per-mode selection-rule bound** $\|\widehat{S_{nt} - \omega/2}(k)\| \leq |k|\,\|\widehat\theta(k)\|$ (and its integrated $\dot H^1$ form) is machine-verified as `sqg_selection_rule_bound` and `sqg_selection_rule_Hs1`; this is the universal Calderón–Zygmund precursor that Proposition 6.1 of this paper refines at the gradient maximum via a parity-based argument. **Proposition 6.1's pointwise $\kappa^2\delta^2$ refinement is not itself machine-verified**; its proof (Lemma 6.2) operates at formal-Taylor-plus-parity level of rigor. **The Lean "conditional Theorem 2" chain (`RieszTorus.lean` §§10–14) is withdrawn as circular.** The named hypothesis structures that mirrored the (H-strain)/(H-bdry)/(H-α) labels were logically vacuous — content $\exists c \geq 0$ / $\exists \alpha < 1$, i.e. equivalent to `True` — and were removed from the source on 2026-05-30; the capstone `sqg_regularity_conditional` was renamed `sqg_uniformHs_conditional`, since its content is modus ponens on an *assumed* uniform-in-time $\dot H^1$ bound on the Galerkin sequence, which is essentially the conclusion; and a governing §10 banner in the source states the circularity for every `sqg_regularity_*` theorem, including the full-range interpolation theorem `sqg_regularity_of_aubinLions_via_interpolation`, whose hypotheses likewise assume uniform $\dot H^1$/$\dot H^s$ Galerkin bounds. "Zero `sorry`, no axioms beyond mathlib" remains literally true of the tree; per the withdrawal, it does not mean the regularity content is real.

The classical Fourier-analysis content — the quantitative uniform-in-$N$ Kato–Ponce commutator bound on $\mathbb{T}^2$, Bony paraproducts, and the $\dot H^s \hookrightarrow L^\infty$ Sobolev embedding via the lattice-zeta constant — is delivered by the companion package [`sqg-lean-proofs-fourier`](https://github.com/Brsanch/sqg-lean-proofs-fourier) (~2,800 LOC, zero `sorry`) and stands independently of the withdrawn chain, as does the bridge constructor `HasSqgGalerkinAllSBound.ofClassical` in `SqgIdentity/FourierBridge.lean`, which assembles $L^2$ conservation, Riesz velocity preservation (constant $C = 1$ via the perp-Riesz mode-by-mode identity), the $\dot H^s$ energy identity (from the parametric-$s$ Galerkin energy machine of `RieszTorus.lean` §10.178–§10.180), the velocity $L^\infty$ bound via Sobolev embedding, the quantitative Kato–Ponce commutator from the companion, and exponential Grönwall closure into uniform $\dot H^s$ bounds on the finite-Fourier-support Galerkin class with uniform-$\ell^\infty$ coefficients. That is a class on which regularity is immediate; the chain is lifted off it only by *assuming* uniform $\dot H^s$ bounds on the Galerkin sequence — i.e. by assuming the conclusion — so none of this bears on regularity for general smooth initial data. The genuine obstruction there is the log-criticality $\|S\|_\infty \lesssim G \log G$ (Status banner).
